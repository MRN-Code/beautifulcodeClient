...
install:
	...
	cp ${PHP_SCRIPT_NAME} ${PHP_SCRIPT_DIR}${PHP_SCRIPT_SUBDIR}${PHP_SCRIPT_NAME} ; \
    if [ -a ${PHP_SCRIPT_DIR}${PHP_SCRIPT_SUBDIR}${PHP_SCRIPT_NAME} ] ; \
	then \
		echo "copied ${PHP_SCRIPT_NAME} to ${PHP_SCRIPT_DIR}${PHP_SCRIPT_SUBDIR}${PHP_SCRIPT_NAME}" ; \
	fi;	
	cp ${SHELL_PROFILE_FILE} ${SHELL_PROFILE_DIR}${SHELL_PROFILE_FILE} ; \
    if [ -a ${SHELL_PROFILE_DIR}${SHELL_PROFILE_FILE} ] ; \
	then \
		echo "copied ${SHELL_PROFILE_FILE} to ${SHELL_PROFILE_DIR}${SHELL_PROFILE_FILE}" ; \
	fi;	
